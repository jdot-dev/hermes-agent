...
class TestDelegationRoutes(unittest.TestCase):
    ...
    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    def test_single_route_preserves_live_manifest_identity(
        self, mock_creds, mock_cfg
    ):
        mock_cfg.return_value = {
            "routes": {
        ...
        with (
            ...
            MockAgent.return_value = child
            delegate_task(
                goal="route work", route="local", parent_agent=parent
            )
        self.assertEqual(mock_live.call_args.kwargs["model"], "local-model")
        self.assertEqual(mock_live.call_args.kwargs["provider"], "local-provider")

    @patch("tools.delegate_tool._load_config")
    @patch("tools.delegate_tool._resolve_delegation_credentials")
    def test_mixed_routes_mark_live_manifest_identity_mixed(
        self, mock_creds, mock_cfg
    ):
        mock_cfg.return_value = {
            "routes": {
        ...
        with (
            ...
            delegate_task(
                tasks=[
                    {"goal": "Run local route work", "route": "local"},
                    {"goal": "Run hosted route work", "route": "hosted"},
                ],
                parent_agent=parent,
            )
        self.assertEqual(mock_live.call_args.kwargs["model"], "mixed")
        self.assertEqual(mock_live.call_args.kwargs["provider"], "mixed")
