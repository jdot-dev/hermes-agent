...
def test_ssh_worker_keeps_remote_cwd_and_drops_gateway_marker(monkeypatch, tmp_path):
    """Remote workers must not inherit a host workspace or gateway identity."""
    root = tmp_path / ".hermes"
    ...
    root.joinpath("config.yaml").write_text("toolsets:\n  - kanban\n", encoding="utf-8")
    monkeypatch.setenv("HERMES_HOME", str(root))
    monkeypatch.setenv("TERMINAL_CWD", "/data/data/com.termux/files/home")
    ...
    assert captured["cwd"] == str(workspace)
    assert captured["env"]["TERMINAL_CWD"] == "/data/data/com.termux/files/home"
